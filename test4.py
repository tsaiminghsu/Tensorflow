...
class PSO_Class:
    ...
    def update(self):
        for i in range(par_num):
            for j in range(par_dim):
                self.particle[i].v[j] = w * self.particle[i].v[j] + \
                                        ( c1 * np.random.rand(1) * ( self.particle[i].pb[j] - self.particle[i].x[j]) )[0] + \
                                        ( c2 * np.random.rand(1) * ( self.gb[j] - self.particle[i].x[j]) )[0]
